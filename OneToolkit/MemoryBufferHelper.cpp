...
namespace winrt::OneToolkit::Storage::implementation
{
	...
	uint8_t MemoryBufferHelper::GetAt(IMemoryBufferReference const& reference, uint32 index)
	{
	...
	void MemoryBufferHelper::SetAt(IMemoryBufferReference const& reference, uint32 index, uint8 value)
	{
		if (!IsIndexValid(reference, index)) throw hresult_out_of_bounds();
		else
		{
	...
	bool MemoryBufferHelper::IsIndexValid(IMemoryBufferReference const& reference, uint32 index)
	{
