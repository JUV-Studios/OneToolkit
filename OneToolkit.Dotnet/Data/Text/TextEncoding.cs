...
namespace OneToolkit.Data.Text
{
	...
	public static class TextEncodingHelper
	{
		...
		public static Encoding Detect(ReadOnlySpan<byte> buffer)
		{
			...
			else if (buffer.Length >= 3 && buffer[0] == ByteOrderMark.Utf8[0] && buffer[1] == ByteOrderMark.Utf8[1] && buffer[2] == ByteOrderMark.Utf8[2]) return Encoding.UTF8;
			else if (IsUtf8(buffer)) return IsAscii(buffer) ? Encoding.ASCII : Encoding.UTF8;
			else if (IsUtf16(buffer, out Endianness endianness)) return endianness == Endianness.Little ? Encoding.Unicode : Encoding.BigEndianUnicode;
			else return null;
		}
	}
}